...
usage() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}📋 LOG GÖRÜNTÜLEYICI - KULLANIM${NC}"
    echo -e "${BLUE}================================================${NC}"
    echo -e "${GREEN}Kullanım:${NC} ./view_logs.sh [komut]"
    echo ""
    ...
    echo -e "  ./view_logs.sh tail"
    echo -e "  ./view_logs.sh live"
    echo -e "  ./view_logs.sh errors"
    echo -e "${BLUE}================================================${NC}"
}
...
case $COMMAND in
    ...
    stats)
        echo -e "${BLUE}================================================${NC}"
        echo -e "${BLUE}📊 LOG İSTATİSTİKLERİ${NC}"
        echo -e "${BLUE}================================================${NC}"
        ...
        # Bugünün çalışma sayısı
        TODAY=$(date +%Y-%m-%d)
        TODAY_RUNS=$(grep -c "FIXTURE 2X SİSTEMİ BAŞLADI" "$LOG_FILE" 2>/dev/null | grep "$TODAY" | wc -l)
